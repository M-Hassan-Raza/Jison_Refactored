...
execFile('jison', [process.argv[2]], function (error) {
  ...
  const fileName = process.argv[2].replace(/(.jison|.json)/, '')
  ...
  function jsPerformActionToCs (str, isLex) {
    ...
    if (isLex) {
          ...
          const symbol = symbols[arguments[3]]
          if (symbol) {
  ...
  const ClassName = FileName.replace(/^.*[\\\/]/, '').charAt(0).toUpperCase() + FileName.replace(/^.*[\\\/]/, '').slice(1)
  const option = {
    	using: '',
    namespace: 'Jison',
    class: ClassName,
  ...
  let parserRaw = fs.readFileSync(__dirname + '/Template.cs', 'utf8')
