...
$(function () {
  ...
  $('.action, .state').on('click', function (ev) {
    if (!$(ev.target).is('a')) { $(this).toggleClass('open') }
  })

  $('.action, .state').on('dblclick', function (ev) {
    const row = this.className.match(/(row_[0-9]+)/)[1]
    const elements = $('.' + row)
    elements.hasClass('open') ? elements.removeClass('open') : elements.addClass('open')
    return false
  })
...
function processGrammar () {
  ...
  parser = new Jison.Generator(cfg, { type, noDefaultResolve: true })
