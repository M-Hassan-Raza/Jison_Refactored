...
const options = { type: 'slr', moduleType: 'commonjs', moduleName: 'jsonparse' }

exports.main = function main (args) {
  const cwd = fs.path(fs.cwd())
  const code = new Parser(exports.grammar, options).generate()
  const stream = cwd.join(options.moduleName + '.js').open('w')
  stream.print(code).close()
}

if (require.main === module.id) { exports.main(system.args) }
