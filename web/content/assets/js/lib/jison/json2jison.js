...
function genHandles (handle, options) {
  if (typeof handle === 'string') {
    ...
    for (let i = 0; i < handle.length; i++) {
      ...
      if (typeof handle[i + 1] !== 'undefined') { s += '\n    | ' }
    }
    return s
  }
}
...
function genLex (lex) {
  ...
  if (lex.macros) {
    for (const macro in lex.macros) {
      if (lex.macros.hasOwnProperty(macro)) {
...
exports.main = function main (args) {
  ...
  if (grammar.bnf) {
    const fname = fs.path(fs.cwd()).join(gfile.basename('.json') + '.jison')
    const stream = fname.open('w')
