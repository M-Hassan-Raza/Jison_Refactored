...
// expand macros and convert matchers to RegExp's
  ...
  function buildActions (dict, tokens) {
    const actions = [dict.actionInclude || '']
    let tok
    const toks = {}
  ...
  RegExpLexer.prototype = {
    ...
    generate: function generate (opt) {
      let code = ''
      if (opt.commonjs) { code = this.generateCommonJSModule(opt) } else { code = this.generateModule(opt) }

      return code
    },
    generateModule: function generateModule (opt) {
      opt = opt || {}
      let out = '/* Jison generated lexer */'
      const moduleName = opt.moduleName || 'lexer'
      out += '\nvar ' + moduleName + ' = (function(){var lexer = ({'
      const p = []
      for (const k in RegExpLexer.prototype) {
        if (RegExpLexer.prototype.hasOwnProperty(k) && k.indexOf('generate') === -1) { p.push(k + ':' + (RegExpLexer.prototype[k].toString() || '""')) }
      }
      out += p.join(',\n')
      out += '})'
    ...
    generateCommonJSModule: function generateCommonJSModule (opt) {
      opt = opt || {}
      let out = '/* Jison generated lexer as commonjs module */'
      const moduleName = opt.moduleName || 'lexer'
