...
const jisonlex = (function () {
  const parser = {
    ...
    performAction: function anonymous (yytext, yyleng, yylineno, yy) {
      const $$ = arguments[5]; const $0 = arguments[5].length
      switch (arguments[4]) {
        case 1:
          this.$ = { rules: $$[$0 - 6 + 4 - 1] }
          if ($$[$0 - 6 + 1 - 1]) {
    ...
    parse: function parse (input) {
      const self = this; let stack = [0]; let vstack = [null]; const table = this.table; let yytext = ''; let yylineno = 0; let yyleng = 0; let shifts = 0; let reductions = 0; let recovering = 0; const TERROR = 2; const EOF = 1
        ...
        if (typeof action === 'undefined' || !action.length || !action[0]) {
          if (!recovering) {
            ...
            if (this.lexer.showPosition) {
              parseError.call(this, 'Parse error on line ' + (yylineno + 1) + ':\n' + this.lexer.showPosition() + '\nExpecting ' + expected.join(', '), { text: this.lexer.match, token: this.terminals_[symbol] || symbol, line: this.lexer.yylineno, expected })
            } else {
              parseError.call(this, 'Parse error on line ' + (yylineno + 1) + ": Unexpected '" + this.terminals_[symbol] + "'", { text: this.lexer.match, token: this.terminals_[symbol] || symbol, line: this.lexer.yylineno, expected })
            }
          }
          if (recovering == 3) {
            if (symbol == EOF) {
